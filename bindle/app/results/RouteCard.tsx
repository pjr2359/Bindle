...
import { ArrowRight, Plane, Train, Bus, Clock, DollarSign, Share2, Bookmark } from 'lucide-react';
          ...
          <div className="flex items-center space-x-2">
            {segments.map((segment, idx) => (
              <React.Fragment key={idx}>
                <div className={`rounded-full p-1 ${segment.type === 'flight' ? 'bg-blue-100 text-blue-600' :
                    segment.type === 'train' ? 'bg-green-100 text-green-600' :
        ...
        {segments.map((segment, idx) => (
          <div key={idx} className="px-6 py-4">
            <div className="flex items-start">
              <div className={`mr-4 rounded-full p-2 ${segment.type === 'flight' ? 'bg-blue-100 text-blue-600' :
                  segment.type === 'train' ? 'bg-green-100 text-green-600' :
                    'bg-yellow-100 text-yellow-600'
                }`}>
                {transportIcon[segment.type]}
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="text-right">
                    <div className="font-medium">${segment.price}</div>
                    <div className="text-sm text-gray-500">{segment.provider}</div>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-2">
                  <div className="flex items-center">
                    ...
                    <div className="h-px w-24 bg-gray-300 mx-2"></div>
                    <div className="h-2 w-2 rounded-full bg-blue-600"></div>
                  </div>
